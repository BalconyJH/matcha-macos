...
/// opening a port. A local interoperability run supplies the receiver URL, Matcha
/// API port, shared token, and simulated bot ID explicitly.
@Suite("Milky Live Interoperability")
struct MilkyLiveInteropTests {
    @Test(
        "a real NoneBot consumer receives an event and calls Matcha API to echo it",
        .enabled(if: ProcessInfo.processInfo.environment["MATCHA_LIVE_MILKY_WEBHOOK_URL"] != nil)
    )
    func realNoneBotEchoRoundTrip() async throws {
        let environment = ProcessInfo.processInfo.environment
        let keys = [
            "MATCHA_LIVE_MILKY_WEBHOOK_URL",
            "MATCHA_LIVE_MILKY_API_PORT",
            "MATCHA_LIVE_MILKY_TOKEN",
            "MATCHA_LIVE_MILKY_SELF_ID",
        ]
    ...
    @Test(
        "a real NoneBot WebSocket consumer discovers the bot and echoes through Matcha API",
        .enabled(
            if: ProcessInfo.processInfo.environment["MATCHA_LIVE_MILKY_EXPECT_WEBSOCKET"] == "1"
        )
    )
    func realNoneBotWebSocketRoundTrip() async throws {
        let environment = ProcessInfo.processInfo.environment
        guard environment["MATCHA_LIVE_MILKY_EXPECT_WEBSOCKET"] == "1" else { return }
        let portText = try #require(environment["MATCHA_LIVE_MILKY_API_PORT"])
        let token = try #require(environment["MATCHA_LIVE_MILKY_TOKEN"])
        let selfID = try #require(environment["MATCHA_LIVE_MILKY_SELF_ID"])
        let port = try #require(UInt16(portText))
