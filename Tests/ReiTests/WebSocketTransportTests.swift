...
struct WebSocketTransportTests {
    ...
    func changingBotIdentityRestartsActiveSessionWithNewSelfID(
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("Rei-BotIdentityTests-\(UUID().uuidString)", isDirectory: true)
        ...
        let defaultsName = "dev.rei.tests.bot-identity.\(UUID().uuidString)"
        ...
        do {
            ...
            if version == .v11 {
                ...
                #expect(capturedHeaders.first?["Sec-WebSocket-Protocol"] == "12.rei")
                #expect(capturedHeaders.last?["Sec-WebSocket-Protocol"] == "12.rei")
            }
    ...
    func failedAppSessionDoesNotReportMessageSuccess() async throws {
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("Rei-FailedSessionTests-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        let defaultsName = "dev.rei.tests.failed-session.\(UUID().uuidString)"
        let defaults = try #require(UserDefaults(suiteName: defaultsName))
        let store = try ReiStore()
        let user = User(
            id: "10001",
    ...
    func reverseConnectionUsesVersionSpecificHandshake(_ version: OneBotVersion) async throws {
        ...
        do {
            ...
            #expect(requestHeaders["User-Agent"] == "Rei/\(ReiVersion.current)")
            ...
            case .v12:
                ...
                #expect(requestHeaders["Sec-WebSocket-Protocol"] == "12.rei")
            }
            ...
            case .v12:
                ...
                #expect(payload["version"]?["impl"]?.stringValue == "rei")
                #expect(payload["version"]?["version"]?.stringValue == ReiVersion.current)
                ...
                #expect(bot["self"]?["platform"]?.stringValue == "rei")
...
private struct EmptyAssetResolver: AssetResolver {
    ...
    func asset(forIdentifier identifier: String, kind: ReiOneBot.AssetKind) async -> Asset? { nil }

    func asset(
        forReference reference: String,
        url: String?,
        kind: ReiOneBot.AssetKind
    ) async -> Asset? { nil }
}
