...
struct LoggingTests {
    ...
    func pipelinesSharingADirectorySerializeFilesAndDisambiguateSequences() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("Rei-LoggingTests-\(UUID().uuidString)", isDirectory: true)
    ...
    func rotationExportsRecordsFromOldestToNewest() async throws {
        ...
        ).filter { $0.lastPathComponent.hasPrefix("Rei.jsonl") }
    ...
    func clearRemovesRuntimeActiveAndEveryRotationWithoutStaleReappearance() async throws {
        ...
        let staleRotation = fixture.log.logDirectory.appendingPathComponent("Rei.jsonl.99")
    ...
    func exportIncludesDiskHistoryRuntimeWindowAndCounts() async throws {
        ...
        let logURL = try #require(urls.first { $0.lastPathComponent == "rei-logs.jsonl" })
        let summaryURL = try #require(urls.first { $0.lastPathComponent == "rei-log-summary.json" })
    ...
    func repeatedExportsCommitIntoDistinctCompleteDirectories() async throws {
        ...
        let firstLogURL = try #require(
            firstURLs.first { $0.lastPathComponent == "rei-logs.jsonl" }
        )
        ...
        for directory in [firstDirectory, secondDirectory] {
            ...
            #expect(names == ["rei-logs.jsonl", "rei-log-summary.json"])
        }
    }
    ...
    func exportSurfacesUnreadableLogGeneration() async throws {
        ...
        defer { fixture.remove() }
        fixture.log.record(.applicationStarted)
        let activeLog = fixture.log.logDirectory.appendingPathComponent("Rei.jsonl")
