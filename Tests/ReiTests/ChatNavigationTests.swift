...
struct ChatNavigationTests {
    ...
    func personaAssignmentsPersist() throws {
        let defaultsName = "dev.rei.tests.persona-settings.\(UUID().uuidString)"
    ...
    private func makeFixture() throws -> ChatNavigationFixture {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("Rei-ChatNavigationTests-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        let log = AppLog(directory: root, emitUnifiedLog: false)
        let defaultsName = "dev.rei.tests.chat-navigation.\(UUID().uuidString)"
