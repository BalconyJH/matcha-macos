...
public enum ReiSettingsTab: Hashable, Sendable {
    case connection
    case roles
    case about
}
...
public struct SettingsView: View {
    ...
    @Binding private var selectedTab: ReiSettingsTab
    @State private var draft: SettingsDraft
    @State private var isSaving = false

    public init(
        environment: AppEnvironment,
        selectedTab: Binding<ReiSettingsTab>
    ) {
    ...
    public var body: some View {
        VStack(spacing: 0) {
            TabView(selection: $selectedTab) {
                ...
                .tag(ReiSettingsTab.roles)

                AboutTab()
                    .tabItem { Label("About", systemImage: "info.circle") }
                    .tag(ReiSettingsTab.about)
            }
...
private struct ConnectionSettingsTab: View {
        ...
        Form {
            Section("Protocol") {
                ...
                if protocolChoice != .milky {
                    ...
                    Text(
                        "Rei serves the Milky API and event WebSocket together. WebHooks below are optional additional event destinations."
                    )
                    .font(.callout)
    ...
    private var connectionModeHelp: String {
        return "Choose whether the framework connects to Rei or Rei connects to the framework"
    }

    private var hostHelp: String {
        switch settings.transport {
        case .webSocketClient:
            return "The NoneBot address that Rei connects to"
        case .webSocketServer:
            return "The address where Rei accepts framework WebSocket connections"
        case .milkyService:
            return "The address where Rei exposes the Milky protocol service"
        }
    }
    ...
    private var portHelp: String {
        switch settings.transport {
        case .milkyService:
            return "Milky uses this Rei port for both its API and /event WebSocket"
    ...
    private var reverseConnectionHelp: String {
        ...
        case .oneBotV11:
            return
                "For a reverse connection, Rei connects to this address. NoneBot V11 must use ReverseDriver and listen on the same host, port, and path."
        case .oneBotV12:
            return
                "For a reverse connection, Rei connects to this address. NoneBot V12 must use an ASGI driver, such as ~fastapi, and listen on the same host, port, and path."
        case .milky:
            return ""
        }
    }
...
private struct AboutTab: View {
    var body: some View {
        Form {
            Section {
                LabeledContent("Rei for macOS", value: "QQ platform simulator and bot framework debugger")
                LabeledContent("Supported Protocols", value: "OneBot V11 / V12, Milky 1.3")
            } footer: {
                Text(
                    "Rei acts as the chat platform, allowing a bot framework to connect and drive complete conversation flows. All personas and messages are generated locally."
                )
                .font(.callout)
                .foregroundStyle(.secondary)
            }
