...
/// means Rei must hold coherent group state to emit even one message event.
struct MilkyEntityEncoder: Sendable {
    ...
    func incomingMessage(_ message: Message, segments: [JSONValue]) async -> JSONValue {
        ...
        case .temp:
            // `group` is optional for temp messages; Rei does not track which
    ...
    func groupNotification(_ request: PendingRequest, seq: Int64) -> JSONValue {
        [
            // Rei currently models join requests and invitations addressed to the
