...
extension MilkyProtocolImplementation {
    ...
    func getImplInfo() -> ProtocolReply {
        .success([
            "impl_name": "matcha",
            "impl_version": "0.1.0",
            // Matcha simulates the platform rather than driving a real QQ client, so
    ...
    func getResourceTempURL(_ call: ProtocolCall) async throws -> ProtocolReply {
        ...
        // answer; for bytes that only ever existed inline, the `matcha-asset://` form
    ...
    func kickGroupMember(_ call: ProtocolCall) async throws -> ProtocolReply {
        ...
        // `reject_add_request` blocklists the member against rejoining. Matcha keeps no
