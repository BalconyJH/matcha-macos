...
///
/// `MatchaStore` is a Sendable value-facing facade. Its SwiftData container,
/// contexts, and reference-model instances remain confined to `StoreExecutor`.
public final class MatchaStore: Sendable {
    let executor: StoreExecutor

    /// Opens a SwiftData store at `path`, creating and migrating it as needed.
    public init(path: String) throws {
        executor = StoreExecutor(modelContainer: try MatchaContainerFactory.persistent(path: path))
    }

    /// An isolated in-memory store, for tests and previews.
    public init() throws {
        executor = StoreExecutor(modelContainer: try MatchaContainerFactory.inMemory())
    }

    /// The store in the user's Application Support directory.
    ///
    /// The name intentionally differs from the former GRDB `matcha.sqlite`: a raw
    ...
    public static func defaultLocation() throws -> MatchaStore {
        ...
        .appendingPathComponent("Matcha", isDirectory: true)

        try FileManager.default.createDirectory(at: base, withIntermediateDirectories: true)
        return try MatchaStore(path: base.appendingPathComponent("matcha.store").path)
    }
...
enum MatchaContainerFactory {
    private static var schema: Schema { Schema(versionedSchema: MatchaSchemaV1.self) }
    ...
    static func persistent(path: String) throws -> ModelContainer {
        ...
        return try ModelContainer(
            for: schema,
            migrationPlan: MatchaMigrationPlan.self,
