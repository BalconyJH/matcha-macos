...
struct OneBotEventEncoder: Sendable {
    ...
    private func encodeRequest(_ request: PendingRequest, time: Int64) -> JSONValue? {
        ...
        case .v12:
            ...
            // Matcha's own shape, named to avoid colliding with a future standard one.
            return base(time: time, type: "request", detailType: "matcha.\(request.kind.rawValue)")
                .merging([
    ...
    private func selfObject() -> JSONValue {
        ["platform": "matcha", "user_id": .string(selfID)]
    }
