...
/// Protocol-specific metadata for a WebSocket connection Rei initiates.
///
...
public protocol ProtocolImplementation: AnyObject, Sendable {
    ...
    /// Metadata used when Rei dials a WebSocket application endpoint.
...
/// Which network role Rei assumes for the selected protocol implementation.
public enum TransportMode: String, Codable, Sendable, CaseIterable, Identifiable {
    /// Rei (the OneBot implementation) listens and the framework connects.
    /// OneBot calls this a forward WebSocket connection.
    case webSocketServer
    /// Rei dials the framework's WebSocket server. OneBot calls this a reverse
    /// WebSocket connection.
    case webSocketClient
    /// Rei runs the Milky protocol service. `/api/*` and `/event` are always
